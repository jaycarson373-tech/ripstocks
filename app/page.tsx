...
const DEFAULT_STOCKDROP_MINT="GUmtbfXHEKyB1SwpyevMHwXa9nx2LT9gKD1a738Vpump";
const STOCKDROP_MINT=(process.env.NEXT_PUBLIC_STOCKDROP_MINT||DEFAULT_STOCKDROP_MINT).trim();
const X_URL=(process.env.NEXT_PUBLIC_X_URL||"https://x.com/StockDrop").trim();
const JUPITER_BUY_URL=`https://jup.ag/?sell=So11111111111111111111111111111111111111112&buy=${STOCKDROP_MINT}`;
const DEXSCREENER_URL=`https://dexscreener.com/solana/${STOCKDROP_MINT}`;
...
export default function Home() {
  ...
  async function copyCa() {
    try {
      await navigator.clipboard.writeText(STOCKDROP_MINT);
      setCopiedCa(true);
  ...
  return (
    <main>
      ...
      <nav className="nav wrap">
        <a className="brand brandImage" href="#top" aria-label="StockDrop home"><img src="/stockdrop-logo.svg" alt=""/><span><em>stock</em>drop</span></a>
        ...
        <button className="caPill headerCa" type="button" onClick={()=>void copyCa()} aria-label="Copy StockDrop contract address"><span>CA</span>{STOCKDROP_MINT}<b>{copiedCa?"COPIED":"COPY"}</b></button>
      ...
      <div className="brandBanner wrap"><img src="/stockdrop-banner.svg" alt="StockDrop — tokenized stock airdrops"/></div>
      <section className="hero wrap" id="top">
        <div className="heroCopy">
          <div className="eyebrow"><span /> STOCKDROP LIVE ON SOLANA</div>
          <h1>HOLDERS.<br/><em>GET DROPS.</em></h1>
          <p>Every {AIRDROP_INTERVAL_MINUTES} minutes, one lucky holder is selected to receive a funded xStock airdrop from the StockDrop treasury.</p>
          <p className="heroSupport">{HOLDER_TICKET_TOKENS.toLocaleString()} DROP = 1 ticket. Draws use a public blockhash seed and post proof after payout.</p>
          ...
          <div className="proof"><div><b>{HOLDER_TICKET_TOKENS/1000}K</b><span>DROP PER TICKET</span></div><div className="nextDrop"><b>{countdown}</b><span>NEXT STOCK DROP</span></div><div><b>{displayedDropCount}</b><span>STOCKS AIRDROPPED</span></div></div>
        </div>
        <div className="machine caseMachine" aria-label="Animated StockDrop drop machine">
          <div className="machineTop"><span>STOCKDROP SELECTOR</span><i>LIVE</i></div>
          <div className="window">
            ...
            <div className="caseResult"><small>LATEST WIN</small><b>{latestDrop?.stock || "DROP"}</b><em>{latestDrop?`$${Number(latestDrop.value).toFixed(2)}`:"ARMED"}</em></div>
          </div>
          <div className="belt">{[1,2,3,4,5,6].map(n=><span key={n} />)}</div>
          <div className="machineBase"><span>FEES FUND DROPS</span><b>→</b><span>HOLDERS WIN</span></div>
        </div>
      </section>
      ...
      <section className="winUniverse wrap" aria-label="Approved xStocks in the StockDrop case">
      ...
      <section className="howWorks wrap" id="how" aria-labelledby="how-title">
        <div className="howIntro">
          ...
          <h2 id="how-title">Hold DROP.<br/>Wait for the airdrop.</h2>
          <p>No paid pack mint is needed for the main utility. The protocol funds stock drops, snapshots holders, selects one lucky wallet, and posts the proof after each confirmed drop.</p>
        </div>
        <div className="howGrid">
          {[
            ["01","HOLD DROP",`${HOLDER_TICKET_TOKENS.toLocaleString()} DROP equals one weighted ticket for the next draw.`],
            ["02","TREASURY FUNDS STOCKS","Protocol fees refill the treasury with approved Solana xStock drops."],
            ["03","15-MINUTE SNAPSHOT","Every 15 minutes, eligible holders are counted and the draw seed is locked."],
            ["04","ONE LUCKY HOLDER","The live selector resolves to one funded xStock drop from the approved universe."],
      ...
      <section className="packs wrap" id="draw">
        <div className="liveStats">{[
          ...
          ["DROP PER TICKET",HOLDER_TICKET_TOKENS,false],
        ...
        <div className="sectionHead"><div><span className="kicker">LIVE HOLDER DROP</span><h2>One stock.<br/>Every 15 minutes.</h2></div><p>Fees stock the treasury with xStock drops from $1 to $50. The draw picks one weighted holder, runs the selector, sends the winning stock drop, and posts proof.</p></div>
        <div className="gachaTeaser" aria-label="Gacha packs coming soon">
          <div className="teaserPack"><img src="/stockdrop-logo.svg" alt=""/><span>LIVE</span></div>
          <div>
            <span className="kicker">STOCKDROP TREASURY</span>
            <h3>One random holder receives one random funded stock drop.</h3>
            <p>The live launch is focused on holder airdrops, treasury-funded xStocks, and public proof every 15 minutes.</p>
          </div>
          <b>ACTIVE</b>
        </div>
        <div className="ripBar drawBar">
          <div><span>NEXT STOCK DROP</span><b>{countdown}</b></div><div><span>ENTRY</span><b>{HOLDER_TICKET_TOKENS.toLocaleString()} DROP = 1 TICKET</b></div><a href="#live">WATCH LIVE ROOM <span>→</span></a>
        </div>
      ...
      <section className="fly wrap" id="flywheel"><span className="kicker">PROOF ENGINE</span><h2>Fair seed.<br/><em>On-chain receipt.</em></h2><div className="protocolSteps">{[["01","HOLDER SNAPSHOT",`${HOLDER_TICKET_TOKENS.toLocaleString()} DROP equals one draw ticket. More tickets means more weight, not a guaranteed win.`],["02","FUNDED INVENTORY","Protocol fees are used to stock the treasury with tokenized stock drops from $1 to $50."],["03","PUBLIC SEED","Each draw combines the 15-minute epoch, a public Solana blockhash, and the holder snapshot hash."],["04",`EVERY ${AIRDROP_INTERVAL_MINUTES} MINUTES`,"One weighted holder is selected and StockDrop resolves to one funded xStock drop."],["05","PUBLISHED PROOF","The winner, stock, amount, transaction, and fairness seed are published after payout."]].map(s=><div className="hourStep" key={s[0]}><b>{s[0]}</b><span>{s[1]}</span><p>{s[2]}</p></div>)}</div>
      <div className="dropProof"><div className="proofTitle"><div><span className="liveDot"/> STOCKDROP PROOFS</div><b>NEXT DROP {countdown}</b></div><div className="proofRows"><div className="proofRow proofLabels"><span>WINNER</span><span>DROP</span><span>STOCK</span><span>VALUE</span><span>SEED</span><span>TX PROOF</span></div>{stockProofs.map((a,i)=><div className="proofRow" key={a.signature||i}><span>{short(a.winner)}</span><span>{proofPackLabel(a.value)}</span><span><b>{a.stock}</b></span><span>${Number(a.value).toFixed(2)}</span><span>{a.randomSeed?short(a.randomSeed):new Date(a.time).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</span><span><a href={`https://solscan.io/tx/${a.signature}`} target="_blank" rel="noreferrer">{short(a.signature)} ↗</a></span></div>)}{stockProofs.length===0&&<div className="emptyProof">No StockDrop holder drops published yet.</div>}</div></div><p className="disclaimer">StockDrop draws are statistical holder rewards funded by treasury inventory. 250k DROP equals one ticket. EV is a statistical expected value calculated from available inventory; it is not a promise of profit.</p></section>

      <section className="verifiedUniverse wrap" aria-labelledby="verified-title"><div className="verifiedHead"><div><span className="kicker">WHICH STOCKS CAN DROP?</span><h2 id="verified-title">10 verified xStocks.<br/>Loaded for airdrops.</h2></div><p>StockDrop inventory is restricted to this approved Solana xStock universe. Every draw resolves to one treasury-funded stock drop.</p></div><div className="verifiedGrid">{stocks.map((stock,index)=><a key={stock.ticker} href={`https://solscan.io/token/${VERIFIED_XSTOCKS[index].mint}`} target="_blank" rel="noreferrer"><span>{String(index+1).padStart(2,"0")}</span><StockLogo stock={stock} className="verifiedLogo"/><div><b>{stock.ticker}</b><small>{stock.name}</small></div><code>{VERIFIED_XSTOCKS[index].mint.slice(0,8)}…{VERIFIED_XSTOCKS[index].mint.slice(-6)}</code><i>↗</i></a>)}</div></section>

      <footer><div className="wrap"><div className="brand brandImage"><img src="/stockdrop-logo.svg" alt=""/><span><em>stock</em>drop</span></div><button className="caPill footerCa" type="button" onClick={()=>void copyCa()}><span>CA</span>{STOCKDROP_MINT}<b>{copiedCa?"COPIED":"COPY"}</b></button><div className="footerLinks"><a href={X_URL} target="_blank" rel="noreferrer">X</a><a href={DEXSCREENER_URL} target="_blank" rel="noreferrer">DEXSCREENER</a><a href={JUPITER_BUY_URL} target="_blank" rel="noreferrer">BUY $DROP</a></div><span>BUILT ON SOLANA ◈</span></div></footer>
