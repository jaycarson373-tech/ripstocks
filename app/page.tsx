...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="fly wrap" id="flywheel"><span className="kicker">HOW IT WORKS</span><h2>Two wallets.<br/><em>One clear flywheel.</em></h2><div className="protocolSteps">{[["01","RIP A PACK","Open a $10 stock pack and instantly receive one tokenized stock from the Main Treasury."],["02","RESTOCK INVENTORY","The Main Treasury receives pack-sale USDC and replenishes replacement $10 xStock inventory lots."],["03","SPLIT PROTOCOL FEES","Fees enter the Main Treasury. Every 20 minutes, 75% moves to Holder Airdrops while 25% stays reserved for Pack EV."],["04",`EVERY ${AIRDROP_INTERVAL_MINUTES} MINUTES`,"The Holder Airdrop Wallet buys and sends one free stock pack to an eligible holder."],["05","PROOF","Confirmed fee splits, deliveries and restocks are recorded on-chain without exposing treasury addresses in the interface."]].map(s=><div className="hourStep" key={s[0]}><b>{s[0]}</b><span>{s[1]}</span><p>{s[2]}</p></div>)}</div>
