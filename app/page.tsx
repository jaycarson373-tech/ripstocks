...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="fly wrap" id="flywheel"><span className="kicker">HOW IT WORKS</span><h2>Two systems.<br/><em>One clear flywheel.</em></h2><div className="protocolSteps">{[["01","RIP A PACK","Open a $10 stock pack and instantly receive one tokenized stock."],["02","RESTOCK INVENTORY","Pack-sale USDC replenishes replacement $10 xStock inventory lots."],["03","SPLIT PROTOCOL FEES","75% funds Holder Airdrops. 25% adds extra stock inventory and raises Pack EV."],["04",`EVERY ${AIRDROP_INTERVAL_MINUTES} MINUTES`,"One eligible holder receives a free stock pack from the Holder Airdrop Treasury."],["05","PROOF","Confirmed deliveries and restocks are recorded on-chain without exposing treasury addresses in the interface."]].map(s=><div className="hourStep" key={s[0]}><b>{s[0]}</b><span>{s[1]}</span><p>{s[2]}</p></div>)}</div>
