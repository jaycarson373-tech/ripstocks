    ...
    <main>
      ...
      <section className="packs wrap" id="packs">
        <div className="liveStats">{[
          ...
          ["NEXT DROP VALUE",snapshot.nextHolderPackValue,true],
        ...
        ].map(([label,value,currency])=><div key={String(label)}><span>{label}</span><b>{currency?`$${Number(value).toFixed(2)}`:Number(value).toLocaleString()}</b>{label==="NEXT DROP VALUE"&&Number(value)===0?<small>FUNDING</small>:null}</div>)}</div>
      ...
      <section className="fly wrap" id="flywheel"><span className="kicker">HOW IT WORKS</span><h2>Two wallets.<br/><em>One clear flywheel.</em></h2><div className="protocolSteps">{[["01","RIP A PACK","Open a $10 stock pack and instantly receive one tokenized stock from the Main Treasury."],["02","RESTOCK INVENTORY","The Main Treasury receives pack-sale USDC and replenishes replacement $10 xStock inventory lots."],["03","SPLIT PROTOCOL FEES","Fees enter the Main Treasury. Every 20 minutes, 75% moves to Holder Airdrops while 25% stays reserved for Pack EV."],["04",`EVERY ${AIRDROP_INTERVAL_MINUTES} MINUTES`,"The Holder Airdrop Wallet sends a $2, $5 or $10 stock pack based on available treasury value."],["05","PROOF","Every confirmed holder drop publishes its winner, stock, value and transaction proof immediately."]].map(s=><div className="hourStep" key={s[0]}><b>{s[0]}</b><span>{s[1]}</span><p>{s[2]}</p></div>)}</div>
