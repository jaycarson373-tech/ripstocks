...
function holderTicketAtoms() {
  const decimals=Number(process.env.RIPS_TOKEN_DECIMALS||process.env.HOLDER_TOKEN_DECIMALS||6);
...
async function eligibleHolders(){
  ...
  const defaultDecimals=Number(process.env.RIPS_TOKEN_DECIMALS||process.env.HOLDER_TOKEN_DECIMALS||6);
...
function pickWinner(holders:HolderTicket[],epochId:number,randomSource:string):FairDraw {
  ...
  const randomSeed=sha256(`stockrips-rips:${epochId}:${randomSource}:${holderHash}:${totalTickets.toString()}`);
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(!fair)return Response.json({ok:true,skipped:"No eligible holder has at least one 250k RIPS ticket",epochId});
